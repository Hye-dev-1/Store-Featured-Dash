// netlify/functions/crawl.js — CommonJS, no ES modules
...
function isGameCat(e){
  ...
  if(e.category){
    ...
    for(var i=0;i<GAME_CATS.length;i++){if(c.indexOf(GAME_CATS[i])>=0)return true;}
    return false;
  }
  if(e.genre){
    var g=toG(e.genre);
    return ['액션','RPG','전략','퍼즐','캐주얼','시뮬레이션','어드벤처','스포츠','카드','리듬'].indexOf(g)>=0;
  }
  return false;
}

// Main handler — CommonJS format for Netlify Functions v1
exports.handler = async function(event) {
  ...
  if(!CFG[country]){
    return {statusCode:400, headers:headers, body:JSON.stringify({error:'Unknown country: '+country})};
  }

  var apiKey = process.env.ANTHROPIC_API_KEY;
  if(!apiKey){
    return {statusCode:500, headers:headers, body:JSON.stringify({error:'ANTHROPIC_API_KEY not set. Add it in Netlify Site settings > Environment variables.'})};
  }

  var c = CFG[country];
  var u = mkUrl(country);
  console.log('[crawl] start ' + country);
  ...
  try {
    var prompt = 'You are extracting GAME data from app stores for ' + c.name + '.\n\n'
      ...
      + 'Use the FIRST genre-related tag (ignore "싱글 플레이어", "멀티플레이어", "오프라인")\n'
      + 'Mark as tab:"Featured"\n\n'
      + 'FOR EACH GAME: Visit its detail page and extract:\n'
      + '- name: exact official title\n'
      + '- dev: developer name\n'
      + '- genre: PRIMARY CATEGORY from detail page, convert to Korean: 액션/RPG/전략/퍼즐/캐주얼/시뮬레이션/어드벤처/스포츠/카드/리듬\n'
      + '- rating: star rating number (e.g. 4.5)\n'
      + '- icon: app icon URL (mzstatic.com or play-lh.googleusercontent.com)\n'
    ...
    if(!res.ok){
      ...
      console.log('[crawl] API error ' + res.status + ': ' + errBody.substring(0,300));
      return {statusCode:502, headers:headers, body:JSON.stringify({error:'Anthropic API ' + res.status, detail:errBody.substring(0,200)})};
    }
    ...
    if(!jsonMatch){
      console.log('[crawl] no JSON found in response. Preview: ' + fullText.substring(0,300));
      return {statusCode:502, headers:headers, body:JSON.stringify({error:'No JSON in AI response', preview:fullText.substring(0,200)})};
    }
    ...
    for(var i=0;i<asRaw.length;i++){
      ...
      asG.push({
        rank: asG.length+1, name:a.name, icon:a.icon||'', genre:toG(a.genre||''),
        rating:a.rating||0, section:a.label||'', dev:a.dev||'',
        tab:a.tab||'Today', url:a.url||'', category:a.category||'Games',
        priority:a.priority||asG.length+1,
      });
    }
    ...
    for(var i=0;i<gpRaw.length;i++){
      ...
      if(!a.name || isEditorial(a.name)) continue;
      gpG.push({
        rank: gpG.length+1, name:a.name, icon:a.icon||'', genre:toG(a.genre||''),
        rating:a.rating||0, section:a.label||'', dev:a.dev||'',
        tab:a.tab||'Featured', url:a.url||'', category:a.category||'Games',
        priority:a.priority||gpG.length+1,
      });
    }

    // Cross-share icons between stores
    var iconMap = {};
    var all = asG.concat(gpG);
    for(var i=0;i<all.length;i++){
      if(all[i].icon && all[i].name){
        iconMap[all[i].name.toLowerCase().replace(/[\s:]/g,'')] = all[i].icon;
      }
    }
    for(var i=0;i<all.length;i++){
      if(!all[i].icon){
        var k = all[i].name.toLowerCase().replace(/[\s:]/g,'');
        if(iconMap[k]) all[i].icon = iconMap[k];
      }
    }
    ...
    console.log('[crawl] done AS=' + asG.length + ' GP=' + gpG.length);
    ...
    console.log('[crawl] fatal: ' + err.message);
