export default async (req, context) => {
  ...
  if (!CFG[country]) return new Response(JSON.stringify({error:"Unknown country"}), {status:400, headers});

  const apiKey = Netlify.env.get("ANTHROPIC_API_KEY") || process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return new Response(JSON.stringify({error:"No ANTHROPIC_API_KEY"}), {status:500, headers});
  ...
  const prompt = `You are extracting GAME data from app stores for ${c.name}.
...
10-20 games per store. All fields required.`;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return new Response(JSON.stringify({error: "API " + res.status, detail: e.substring(0,200)}), {status:502, headers});
    }
    ...
    if (!m) return new Response(JSON.stringify({error:"No JSON", preview: txt.substring(0,200)}), {status:502, headers});
    ...
    // Cross-share icons
    const ic = {};
    for (const g of [...asG, ...gpG]) { if (g.icon && g.name) ic[g.name.toLowerCase()] = g.icon; }
    ...
    }), { status: 200, headers: { ...headers, "Cache-Control": "public, max-age=3600" } });
